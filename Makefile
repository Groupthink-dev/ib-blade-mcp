...
help:
	...
	@echo "  make test-e2e         - Run e2e tests (requires live IB Gateway)"
...
test-e2e:
	IB_E2E=1 uv run pytest tests/ -v -m e2e

test-cov:
	uv run pytest tests/ -m "not e2e" --cov=src/ib_mcp --cov-report=term-missing -v
